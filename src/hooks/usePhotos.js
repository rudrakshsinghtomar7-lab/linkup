...
// Photos live in the private trip-photos bucket under <album id>/…; rows in trip_photos.
// `album` is { kind: 'trip' | 'plan', id }.
export function usePhotos(album, userId) {
  const tripId = album?.id
  const col = album?.kind === 'plan' ? 'plan_id' : 'trip_id'
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    const { data, error } = await supabase.from('trip_photos').select('*').eq(col, tripId).order('created_at', { ascending: false })
  ...
  const upload = useCallback(async (files) => {
    ...
    for (const file of list) {
      try {
        ...
        const { error: rowErr } = await supabase.from('trip_photos').insert({ [col]: tripId, storage_path: path, uploaded_by: userId, ...dims })
